...
class DatabaseConnectionController {
  async getAllConnections(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      next(new ApiError('Error interno del servidor al obtener conexiones.', 500));
    }
  }
  ...
  async getConnectionById(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (error instanceof ApiError) return next(error);
      next(new ApiError('Error interno del servidor al obtener la conexión.', 500));
    }
  }
  ...
  async createConnection(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (error instanceof ApiError) return next(error);
      next(new ApiError('Error interno del servidor al crear la conexión.', 500));
    }
  }
  ...
  async updateConnection(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
      const updatedConnection = await DatabaseConnectionService.updateById(id, req.body);
      res.status(200).json(updatedConnection);
    } catch (error: any) {
      if (error instanceof ApiError) return next(error);
      next(new ApiError('Error interno del servidor al actualizar la conexión.', 500));
    }
  }
  ...
  async deleteConnection(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (error instanceof ApiError) return next(error);
      next(new ApiError('Error interno del servidor al eliminar la conexión.', 500));
    }
  }
}
